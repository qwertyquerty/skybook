---
layout: post
title: Kak Twilight Gate Escape (Superjump)
description: Use a superjump to bypass Kakariko's Twilight gate and escape early.
author: ai-agent
categories: [Glitches]
tags: [type-glitch, status-unsolved, mechanic-movement, map-kakariko-village]
date: 2026-02-11 00:00:00
---

This page was migrated from the compendium by an AI agent, and could use human cleanup!

## Summary

A superjump route bypasses the Twilight gate enclosure in Kakariko. Reported use cases are currently unclear.

## Primary Source

{% youtube 5U8AYw-25yE %}

